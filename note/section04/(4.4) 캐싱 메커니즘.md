...
- useQuery훅을 통해 백엔드 서버에 요청을 보내 데이터를 불러오게 되면,
  해당 데이터를 쿼리키에 설정해둔 이름으로 자동 캐싱함

- 특정 시간동안 중복된 데이터 요청이 있을때 불필요한 요청을 방지하기 위해서
  캐시데이터를 사용하여 앱의 성능을 최적화에 도움을 줌

- 꼭 최신 데이터를 반영해야 하는 상황에도, 자체 캐싱매커니즘에 의해
  특정 시간 이후에 데이터가 자동으로 갱신 되기도 하고, 또는 삭제되기도 함
- TanStack Query는 사용자에게 최대한 로딩 화면을 덜 보여주는 방식으로 앱의 최적화를 해준다.

# TanStack Query 캐시의 5가지 상태

- fetching : 데이터를 불러와 지는 중일때의 상태
- fresh : 데이터가 신선한 상태일때
- stale : 데이터 시간이 지나 신선하지 않은 상태 (상한 상태)

* fresh → stale : staleTime (따로 설정 가능)
* stale → fresh : refetching
...
- 서버로부터 데이터를 불러오자마자 stale상태로 만들어 사용자가 페이지 에 새롭게 방문할 때마다 언제다 다시 리패칭되도록 설정
